...
(defn pit
  ...
  (let [ajax? (some #(= "+ajax" %) params)
        ...
        deps (cond-> default-dependencies
                               (not ajax?) (conj '[org.clojure/core.async "0.2.395"] '[io.nervous/kvlt "0.1.4"])
                               ajax?       (conj '[cljs-ajax "0.5.8"]))
        data {:name name
              :sanitized (name-to-path name)
              :dependencies (indent 17 (map pr-str deps))
              :api-lib (if ajax? 'ajax 'async)}
        raw-files [;; Project
                   ["project.clj" (render "project.clj" data)]

                   ;; Core files
                   ["src/{{sanitized}}/core.cljs" (render "src/core.cljs" data)]
                   ["dev/dev/core.cljs" (render "src/dev_core.cljs" data)]
